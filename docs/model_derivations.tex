...
and reaches about $0.92$ intraday and $0.92$ daily, and the price bands cover
...
Figure \ref{fig:clustering} makes this visible. The top panel is a simulated
daily return series where calm stretches and turbulent stretches are clearly
separated rather than mixed at random. The lower panels are the autocorrelation
functions that separate the two signals. The autocorrelation of the raw returns
sits inside the noise band at every lag, which is the direction being
unpredictable, while the autocorrelation of the squared returns stays well above
the band and decays only slowly, which is the volatility being predictable. On
this series the raw return autocorrelation averages about $-0.01$ over the first
ten lags while the squared return autocorrelation averages about $0.30$ and is
still near $0.20$ at lag forty.

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{figures/derivations/clustering.png}
\caption{Volatility clustering. The returns cluster into calm and turbulent
regimes, the autocorrelation of returns is inside the significance band and so
carries almost no signal about direction, and the autocorrelation of squared
returns is large and slowly decaying, which is the persistent signal the model
exploits.}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{figures/derivations/stage_data.png}
\caption{The data pipeline, from the ticker universe through the two downloads
and the incremental cache to the liquidity filter, the features, and the cross
sectional rank label.}
\label{fig:stagedata}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{figures/derivations/stage_split.png}
\caption{The purged split on the time axis. The training block ends before the
test block, a purge removes the training rows whose forward label window reaches
into the test period, and an embargo adds a further gap.}
\label{fig:stagesplit}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{figures/derivations/stage_loop.png}
\caption{The training loop for one epoch, shuffling the rows, forming batches,
running the forward pass to the loss, backpropagating through the trunk and
through time, and applying the Adam update, then repeating.}
\label{fig:stageloop}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{figures/derivations/stage_feedback.png}
\caption{The validation feedback. A held out loss decides whether to save a new
best or to advance the patience counter, which halves the learning rate after a
short stall and stops early after a long one.}
\label{fig:stagefeedback}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{figures/derivations/stage_test.png}
\caption{Testing on the held out block. The model is scored by area under the
curve and band coverage, and the result is trusted only if it beats both the
naive persistence baseline and the gradient boosted baseline.}
\label{fig:stagetest}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{figures/derivations/stage_report.png}
\caption{Reporting and saving. The run prints the term structure, the area under
the curve, the coverage, and a price cone, and it writes a checkpoint of the
weights and the scaler for reuse and the ensemble.}
\label{fig:stagereport}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{figures/derivations/stage_tune.png}
\caption{Tuning and modifying. Hyperparameter tuning and design changes both
feed back into the data and feature stages, which is the outer iterate loop that
...
quantile band heads. The gated static context vector is fused alongside them so
the cross sectional hierarchy signal reaches every horizon.}
\label{fig:fusion}
\end{figure}
...
\subsection{The Static Context and the Hierarchy Gate}

The window branches see only the recent price and volume path, which is a thin
signal, so we also fuse a static cross sectional context vector into the trunk.
This vector holds the features a strong cross sectional model relied on, multi
window realized volatility, momentum, and market relative strength, each ranked
across names on the same date so it is market neutral. The market relative
columns carry the hierarchy signal, the position of a stock against the broad
market. Writing the context as $c$ the model scales it by a learned hierarchy
gate before fusing it,
\[
\tilde c = g \odot c, \qquad g = 2\,\sigma(w),
\]
so each feature scale sits in $(0, 2)$ and starts at one, which is neutral. The
gated context is concatenated with the embeddings and the drift, so the fused
vector becomes
\[
\text{fuse} = \bigl[\,e_1,\dots,e_B,\ \Delta_1,\dots,\Delta_{B-1},\ \tilde c\,\bigr].
\]

The gate learns which feature groups matter, which is the same mechanism the
cross sectional network uses, and its gradient is the context error scaled by the
derivative of the gate,
\[
\boxed{\,\frac{\partial \mathcal{L}}{\partial w}
= \sum_n \left(\frac{\partial \mathcal{L}}{\partial \tilde c_n} \odot c_n\right)
  \, g\,\bigl(1 - \tfrac{g}{2}\bigr).}
\]

This is how we turn the hierarchy weighting back on for the term structure model,
so the long horizons can reach the predictability that the cross sectional
features carry rather than the ceiling the price path alone allows.
...
nearly smooth,
\begin{align*}
\text{at } 5\text{d}:\ -0.0831, \quad
\text{at } 10\text{d}:\ +0.0099, \quad
\text{at } 30\text{d}:\ -0.0264, \quad
\text{at } 90\text{d}:\ -0.0226.
\end{align*}
...
$0.64$ at one day to about $0.92$ at the long horizons, and the intraday scale
