...
This project predicts the volatility of a stock with respect to time. Volatility
is the size of the moves a stock makes, so it governs how far the price can
travel up or down over a horizon rather than the sign of that travel. We predict
this quantity as a function of the horizon, which is what we mean by with respect
to time, and we show that it carries a clear and learnable training signal
precisely because volatility clusters intrinsically in time.
...
nearly smooth,
\begin{align*}
\text{at } 5\text{d}:\ -0.1100, \quad
\text{at } 10\text{d}:\ -0.0400, \quad
\text{at } 30\text{d}:\ -0.0400, \quad
\text{at } 90\text{d}:\ +0.0100.
\end{align*}
...
$0.70$ at one day to about $0.97$ at the long horizons, and the intraday scale
climbs from about $0.66$ at five minutes to about $0.90$ at four hours.}
\label{fig:auc}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.68\textwidth]{figures/derivations/conformal.png}
\caption{Conformity scores on the calibration set. The vertical line is the
order statistic $\delta = s_{(k)}$ that widens the band, and it lifts coverage
from $0.68$ to $0.91$.}
\label{fig:conformal}
\end{figure}
...
quantiles, and we project the data by simulating four thousand price paths at the
same volatility. The shaded model cone and the empirical quantiles of the
simulated paths track each other closely, and the fraction of paths that finish
inside the five to ninety five band is $0.899$, which matches the nominal ninety
percent. Working the one hundred eighty day edges through $P = 100\,e^{q}$ gives
\begin{align*}
\text{5th } &\$74.01 \ (\text{data } \$73.74), &
\text{25th } &\$88.39 \ (\text{data } \$88.22), \\
\text{50th } &\$100.00 \ (\text{data } \$100.35), &
\text{75th } &\$113.14 \ (\text{data } \$114.09), \\
\text{95th } &\$135.12 \ (\text{data } \$134.91). & &
\end{align*}
