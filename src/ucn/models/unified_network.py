...
class UnifiedCourseNetwork:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> "UnifiedCourseNetwork":
        cfg = self.cfg
        K   = len(np.unique(y))
        if not self.params:
        ...
        for epoch in range(cfg.epochs):
            ...
            if cfg.verbose and (epoch + 1) % cfg.verbose == 0:
                ...
                val_acc = accuracy(y_val, np.argmax(c_val['Y_hat'], axis=1))
        ...
        if best_params is not None:
            ...
            if cfg.verbose:
                print(f"  Restored best checkpoint (val_CE={best_val:.5f})",
                      flush=True)
        return self
