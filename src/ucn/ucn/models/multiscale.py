...
class MultiScaleTermStructureNet:
    ...
    def fit(self, seq_list, Y, Yret=None):
        ...
        for epoch in range(self.epochs):
            ...
            if bce < best - 1e-6:
                ...
                # so training keeps refining once the loss flattens.
                if plateau >= self.lr_patience and cur_lr > self.min_lr:
    ...
    def predict_bands(self, seq_list):
        """Return conformally calibrated return quantile bands, shape (N, B, Q)."""
        seq_list = [to_device(s) for s in seq_list]
        seq_list = self._apply_scalers(seq_list)
        q = to_cpu(self._forward(seq_list, training=False)["q"])
