...
def make_features(
    ...
    sent_df     : VADER sentiment scores (dates x tickers), optional
    vol_df      : daily volume (dates x tickers), optional
    top_pct     : top cross-sectional quantile for outperformer label
    bottom_pct  : bottom quantile for underperformer label
    min_history : minimum trading days required per ticker

    Returns
    -------
    X_final    : cross-sectionally ranked feature DataFrame
    y_final    : binary labels (1 = outperformer, 0 = underperformer)
    feat_names : list of feature column names
    has_sent   : True if sentiment was appended as the last feature
    """
    ...
    print(f"  Feature matrix: {X_final.shape}  base rate: {y_final.mean():.4f}  "
          f"(kept {keep.sum():,} / {len(fwd_raw):,} rows)", flush=True)
